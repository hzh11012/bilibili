        ...
        <div class="main-container">
          ...
          <div class="center-box">
            <div class="name-line">
              ...
              >{{parseInt((nowTime-item.desc.timestamp*1000)/(1000*60))+' 分钟前'}}</span>
              <span
                class="publish-time"
          ...
          <a href="/" target="_blank" class="right-box">
            <div
              class="cover"
              :style="{backgroundImage:'url('+ (item.desc.uid_type==1?JSON.parse(item.card).image_urls[0]:JSON.parse(item.card).cover) +'@64w_48h_1c.webp)'}"
            >
...
export default {
  ...
  methods: {
    ...
    getNew() {
      ...
      this.$http
        .get(
          "/vc/dynamic_svr/v1/dynamic_svr/dynamic_new?uid=172738743&type_list=64"
        )
        .then((res) => {
      ...
      this.$http
        .get(
          "/vc/dynamic_svr/v1/dynamic_svr/dynamic_history?uid=" +
            ...
            "&type_list=64&offset_dynamic_id=" +
            this.history_offset
        )
        .then((res) => {

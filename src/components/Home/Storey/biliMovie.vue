      ...
      <div class="card-list">
        <header class="storey-title">
          <div class="l-con">
            ...
            <a href="/" target="_blank" class="name">电影</a>
          </div>
          <div class="exchange-btn">
            <div class="btn btn-change" @click="changeMovie">
              <svg class="icon" aria-hidden="true">
        ...
        <div class="zone-list-box">
          <div class="zone-list-box" v-if="ismovie">
            <common-card :showUp="true" :item="item" v-for="(item,index) in movieList" :key="index"></common-card>
          </div>
        </div>
      </div>
      <div class="pgc-rank">
...
export default {
  ...
  methods: {
    getMovie() {
      this.$http
        .get("/api/x/web-interface/dynamic/region?ps=12&rid=23")
        .then((res) => {
          this.movieList = res.data.data.archives;
          this.ismovie = true;
        });
    },
    getRank() {
      this.$http
        .get("/api/pgc/web/rank/list?season_type=2&day=3")
        .then((res) => {
    ...
    handleScroll() {
      let el = document.getElementById("biliMovie");
      let scrollTop =
